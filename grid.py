CELL_EMPTY     = ' ' #Не следует делать ненужные отступы
CELL_PLAYER    = 'X'
CELL_COMPUTER  = 'O'
...
class Grid:
    ...
    def valid_coords(self, i, j):
        return (i >= 0 and i < self.n) and (j >= 0 and j < self.n) #лучше (0 <= i < self.n) and (0 <= j < self.n)
    ...
    def who_won(self):
        for row in range(self.n):
            for col in range(self.n): #То же самое с переименованием
                #Возможно лучше вынести код для игрока в функцию с проверкой и выполнить эту функцию и для 2го игрока
